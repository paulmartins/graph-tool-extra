...
def edge_disjoint_paths(g: gt.Graph, source: gt.Vertex, target: gt.Vertex):
    if int(source) > g.num_vertices() or int(target) > g.num_vertices():
        raise IndexError(f"source ({int(source)}) or target ({int(target)}) vertex index out of range {[0, g.num_vertices()]}")
    ...
    if not g_is_directed:
        ...
        reversed_edges = np.stack([edges_array[:,1], edges_array[:,0]], axis=-1)
        g.add_edge_list(reversed_edges)

    # Edge property to save capacity values
    capacity = g.new_edge_property("int64_t", val=1)
    # for edge in g.vertex(source).out_edges():
    #     capacity[edge] = PSEUDO_INF
    ...
    flow.a = capacity.a - residual.a
    max_flow = sum(flow[e] for e in g.vertex(target).in_edges())
    print(f"Number of edge disjoint paths = {max_flow}")
    ...
    # Find all the paths from source to target in the filtered graph
    # NOTE: one could also use all_shortest_paths
    all_combinations = [path for path in gt.all_paths(gp, target, source)]
    edge_disjoint_paths = np.lib.stride_tricks.sliding_window_view(all_combinations.pop(), 2)
    paths_length = [len(edge_disjoint_paths)]
    while len(all_combinations) > 0:
        current_path = all_combinations.pop()
        edges = np.lib.stride_tricks.sliding_window_view(current_path, 2)
        intersect = set([tuple(row) for row in edges]).intersection([tuple(row) for row in edge_disjoint_paths])
        if len(intersect) == 0:
            paths_length.append(len(edges))
            edge_disjoint_paths = np.concatenate([edge_disjoint_paths, edges])

    # Cleanup to leave g in the same state as it was initially
    g.set_directed(g_is_directed)
    gt.remove_parallel_edges(g)
    
    return np.split(edge_disjoint_paths, np.cumsum(paths_length))
